...
export class Dashboard {
    ...
    this.router.events
      ...
      .subscribe(() => {
        ...
        if (url.includes('user-overview')) {
          ...
          this.currentPage = 'manage-content';
          this.cdr.markForCheck();
        } else if (url.includes('databaseConfiguration')) {
          this.currentPage = 'manage-content';
          this.cdr.markForCheck();
  ...
  onMenuChanged(event: { menu: string, isSubmenu: boolean, mainMenu?: string }) {
    ...
    if (event.menu === 'User Overview') {
      ...
      this.cdr.markForCheck();
    } else if (event.menu === 'Product Management') {
      this.headerTitle = 'Admin Center';
      this.subMenuTitle = 'Work Flow';
      this.isSubmenu = event.isSubmenu;
      this.mainMenu = 'product-management';
      this.cdr.markForCheck();
    } else if (event.menu === 'Sub-Product Management') {
      this.headerTitle = 'Admin Center';
      this.subMenuTitle = 'Work Flow';
      this.isSubmenu = event.isSubmenu;
